...
const separateMediaAndDocs = (
  attachments: Attachment[],
): { media: Attachment[]; docs: Attachment[] } => {
...
const DocumentItem = ({ attachment }: { attachment: Attachment }) => {
  ...
  const fileSizeLabel = attachment.size
...
const AttachmentList = ({ attachments }: AttachmentListProps) => {
  ...
  const { media: mediaItems, docs: docItems } = useMemo(
    () => separateMediaAndDocs(attachments),
    [attachments],
  );

  // Pre-compute image URLs for preview dialog to avoid filtering on every click
  const imageAttachments = useMemo(
    () => mediaItems.filter(isImageAttachment),
    [mediaItems],
  );
  const imageUrls = useMemo(
    () => imageAttachments.map(getAttachmentUrl),
    [imageAttachments],
  );
      ...
      <div className="w-full rounded-lg border border-border bg-muted/20 overflow-hidden">
        ...
        <div className="p-2 flex flex-col gap-1">
          ...
          {mediaItems.length > 0 && docItems.length > 0 && (
            <div className="border-t mt-1 border-border opacity-60" />
          )}
