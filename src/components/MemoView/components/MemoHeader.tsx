...
const MemoHeader: React.FC<MemoHeaderProps> = ({ showCreator, showVisibility, showPinned, onEdit, onGotoDetail, onUnpin }) => {
  ...
  const displayDate = toDate(memo.displayTime);
  const displayTime = isArchived ? (
    displayDate?.toLocaleString(i18n.language)
  ) : (
    <relative-time datetime={displayDate?.toISOString()} lang={i18n.language} format={relativeTimeFormat}></relative-time>
  );
