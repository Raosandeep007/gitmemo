...
import type { Memo, MemoReaction, User } from "@/types/github";
  ...
  const handleReactionClick = async (reactionType: string) => {
    ...
    try {
      if (hasReacted(reactionType)) {
        ...
        await memoService.upsertMemoReaction(memo.name, reactionType);
      }
      // Refetch the memo to get updated reactions and invalidate cache
      const updatedMemo = await memoService.getMemo(memo.name);
