...
import type { MemoRelation, MemoRelationMemo } from "@/types/github";
...
const RelationList: FC<RelationListProps> = ({ relations, onRelationsChange, parentPage, memoName }) => {
  const referenceRelations = relations.filter((r) => r.type === MemoRelationType.REFERENCE && (!memoName || r.memo?.name === memoName));
  const [fetchedMemos, setFetchedMemos] = useState<Record<string, MemoRelationMemo>>({});
    ...
    (async () => {
      ...
      if (missingSnippetRelations.length > 0) {
        const requests = missingSnippetRelations.map(async (relation) => {
          const memo = await memoService.getMemo(relation.relatedMemo!.name);
          return { name: memo.name, snippet: memo.snippet };
        });
  ...
  const handleDeleteRelation = (relatedMemoName: string) => {
    if (onRelationsChange) {
      onRelationsChange(relations.filter((relation) => relation.relatedMemo?.name !== relatedMemoName));
    }
  };
